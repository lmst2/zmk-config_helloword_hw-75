 ...
 *
 * Same as ZMK behavior_rgb_underglow; linked when CONFIG_HW75_EXTENDED_RGB replaces stock RGB.
 */
								  ...
								  struct zmk_behavior_binding_event event)
{
	...
	case RGB_TOG_CMD: {
		bool s;
		int err = zmk_rgb_underglow_get_state(&s);
		if (err) {
			LOG_ERR("RGB get state failed (%d)", err);
			return err;
		}
		binding->param1 = s ? RGB_OFF_CMD : RGB_ON_CMD;
		break;
	}
	case RGB_BRI_CMD: {
